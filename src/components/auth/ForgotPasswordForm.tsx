...
export function ForgotPasswordForm({ initialEmail }: ForgotPasswordFormProps) {
  ...
  async function onSubmit(data: ForgotPasswordInput) {
    ...
    try {
      ...
      if (!response.ok) {
        const nextError =
          payload?.error ?? "Nao foi possivel processar sua solicitacao agora.";
      ...
      const nextMessage =
        payload?.message ??
        "Se existir uma conta valida, enviaremos um e-mail para redefinir sua senha.";
      setServerMessage(nextMessage);
      toast.success(nextMessage);
    } catch {
      const nextError = "Nao foi possivel processar sua solicitacao agora.";
      setServerError(nextError);
      toast.error(nextError);
    } finally {
      setLoading(false);
